...
piece = fromList nodes
  where nodes = [ (front x y, neighbors x y) | x <- range mazeWidth,
                                              y <- range mazeHeight ]
        neighbors x y = [ front nx y | nx <- [x+1,x-1], nx >= 0, nx < mazeWidth ] ++
                        [ front x ny | ny <- [y+1,y-1], ny >= 0, ny < mazeHeight ]
...
createPageContent m starting ending page = drawWithPage page $ do
  withNewContext $ do
    ...
    withNewContext $ do
      setWidth $ 4.0 / s
      stroke $ Line 0 0 width 0
      stroke $ Line width 0 width height
      stroke $ Line width height 0 height
      stroke $ Line 0 height 0 0
...
main = do
    ...
    runPdf "rectangle.pdf" (standardDocInfo { author="Jim Fowler", compressed = False}) rect $ do
